...
class Stage2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        setupTitleAndRightButtonView(title: "Driver-Controlled", isProfile: false)
    ...
    @objc func nextStageAction() {
        if stage2Score == .zero {
            ...
            HavaConstants.stage2Score = stage2Score
            let nav = EndGameViewController(stageModel: StageModel.stage3, stage: 3)
...
extension Stage2ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == .zero {
            ...
            score.text = stage2Score.description
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        DispatchQueue.main.async { [self] in
            ...
            if cell?.checked == true {
                stage2Score += Int("\((stageModel![indexPath.section].pointsValue?.sorted { $0.key > $1.key }[indexPath.row].value)!)")!
            } else {
                stage2Score -= Int("\((stageModel![indexPath.section].pointsValue?.sorted { $0.key > $1.key }[indexPath.row].value)!)")!
            }
            score.text = stage2Score.description
